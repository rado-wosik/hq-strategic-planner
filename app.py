...
# CSS: High contrast for Sidebar + Great visibility for Main Panel
st.markdown("""
    <style>
    /* Main Background */
    .stApp { background-color: #0b0f19; color: #e0e0e0; }
    
    /* SIDEBAR FIX: Permanent white text and clear headers */
    ...
    /* KPI Metric Cards - The ones you liked */
...
# --- 6. ANALYTICS (REINSTATED) ---
st.divider()
cl, cr = st.columns(2)

with cl:
    st.subheader("Seasonal Scenario Analysis")
    if selected_month in ["Dec", "Jan", "Feb"]: st.error("❄️ **WINTER:** Peak heating demand. High wind yields.")
...
st.divider()
st.caption("Data Bridge Engine v2.9 | Strategic Planner | Hydro-Québec Integration")