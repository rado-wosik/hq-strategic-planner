...
# CSS: Better contrast, readable links and headers
st.markdown("""
    <style>
    /* Main Background */
    ...
    /* Sidebar Styling */
    section[data-testid="stSidebar"] { background-color: #161b26; border-right: 1px solid #2d3748; }
    ...
    /* KPI Metric Cards - Improved Visibility */
    ...
    div[data-testid="stMetricValue"] > div {
        color: #00aeef !important; /* Data Bridge Blue for values */
        font-size: 2.2rem !important;
        font-weight: 800 !important;
    }

    /* General Text & Links */
    .stMarkdown, p, span, label { color: #ffffff !important; font-size: 1rem; }
    a { color: #00aeef !important; font-weight: bold; }
    
    /* Fix for slider labels visibility */
    .stSlider label { font-weight: 700 !important; color: #00aeef !important; }
    </style>
...
with st.sidebar:
    ...
    st.markdown('<p style="font-size: 1.4rem; font-weight: 800; margin-top: 0; color: #ffffff;">Strategic Planner</p>', unsafe_allow_html=True)
...
demand[120:] *= 0.88 # Slight weekend drop
...
fig.update_layout(
    ...
    xaxis=dict(
        gridcolor='#2d3748', tickfont=dict(color='#ffffff', size=12),
        tickmode='array', tickvals=time_axis[::24], ticktext=days
    ),
...
with cl:
    ...
    if selected_month in ["Dec", "Jan", "Feb"]: st.error("❄️ **WINTER:** Extreme heating peaks. Wind is the primary green source.")
    elif selected_month in ["Mar", "Apr", "May"]: st.warning("🌊 **SPRING:** Freshet period. High hydro-inflow. Maximize export potential.")
    elif selected_month in ["Jun", "Jul", "Aug"]: st.success("☀️ **SUMMER:** Optimal Solar PV output. Low domestic consumption.")
    else: st.info("🍂 **AUTUMN:** Transition season. Rising evening peaks.")

with cr:
    st.subheader("Weekly Export Profile (GW)")
...
st.divider()
st.caption("Data Bridge Engine v2.6 | Strategic Planner | Hydro-Québec Integration")